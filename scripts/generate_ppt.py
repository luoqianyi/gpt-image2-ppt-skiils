...
""",
    ...
    parser.add_argument(
        "--editable",
        action="store_true",
        help="生成可编辑PPTX：OCR提取文字 → LAMA背景修复 → 重建可编辑文本框（需 pip install px-image2pptx[ocr,inpaint]）",
    )
    parser.add_argument(
        "--skip-inpaint",
        action="store_true",
        help="配合 --editable：跳过LAMA背景修复（更快，但文本框下仍有原文字）",
    )
...
def main() -> None:
    ...
    if not args.no_pptx:
        if args.editable:
            # 可编辑PPTX模式：OCR + 背景修复 + 文本框重建
            sys.path.insert(0, str(SCRIPT_DIR))
            from editable_pptx import run_pipeline as run_editable_pipeline
            pptx_path = run_editable_pipeline(
