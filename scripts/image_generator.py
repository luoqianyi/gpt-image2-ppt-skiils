...
class GptImage2Generator:
    ...
    def __init__(self, aspect_ratio: str = "16:9") -> None:
        ...
        # endpoint: chat | images | edits | auto（auto: 有参考图时 edits->chat，无参考图时 images->chat）
    ...
    # ---------- 端点 3: /v1/images/edits (图片编辑) ----------

    def _request_via_edits(self, prompt: str, size: str, reference_image_path: str) -> str:
        """用 /v1/images/edits 端点编辑图片。

        将参考图作为 image 上传，prompt 描述要做的修改。
        此端点在支持的中转站上可以真正编辑图片（保持布局+替换文字），
        效果优于 chat completions 方式。
        """
        url = f"{self.base_url}/v1/images/edits"
        headers = {
            "Authorization": f"Bearer {self.api_key}",
        }
        print(f"🔗 POST {url}  size={size}  (edits mode)")
        with open(reference_image_path, "rb") as f:
            resp = requests.post(
                url,
                headers=headers,
                files={"image": (os.path.basename(reference_image_path), f, "image/png")},
                data={
                    "model": self.model_name,
                    "prompt": prompt,
                    "n": 1,
        ...
        if resp.status_code != 200:
            raise RuntimeError(
                f"edits 调用失败 (status={resp.status_code}): {resp.text[:500]}"
            )

        result = resp.json()
        data = result.get("data") or []
        if not data:
            raise RuntimeError(f"edits 响应没有 data: {str(result)[:300]}")
        first = data[0]
        b64 = first.get("b64_json")
        url_field = first.get("url")
        if b64:
        ...
        if url_field:
            return url_field
        raise RuntimeError(f"edits data[0] 既没 b64_json 也没 url: {str(first)[:300]}")
                ...
                try:
                    if self.endpoint == "images":
                        if reference_image_path:
                            # 优先尝试 edits 端点，失败则回退 chat
                            try:
                                payload = self._request_via_edits(prompt, target_size, reference_image_path)
                            except Exception as edits_err:
                                print(f"(!) edits 失败，回退到 chat: {str(edits_err)[:120]}")
                        ...
                        payload = self._request_via_chat(prompt, target_size, reference_image_path)
                    elif self.endpoint == "edits":
                        # 强制走 edits 端点（需要 reference image）
                        if reference_image_path:
                            payload = self._request_via_edits(prompt, target_size, reference_image_path)
                        else:
                            payload = self._request_via_images(prompt, target_size)
