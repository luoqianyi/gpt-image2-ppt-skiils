...
GPT_IMAGE_ENDPOINT=auto                    # auto | edits | images | chat（见下方说明）
...
### GPT_IMAGE_ENDPOINT 端点选择说明

| 值 | 行为 | 适用场景 |
| --- | --- | --- |
| `auto`（默认） | 有参考图时 edits→chat 回退链；无参考图时 images→chat 回退链 | 推荐，自动适配中转站能力 |
| `edits` | 强制走 `/v1/images/edits`（需要参考图） | 中转站明确支持 edits 端点时 |
| `images` | 有参考图时 edits→chat；无参考图时走 `/v1/images/generations` | 兼容性好 |
| `chat` | 强制走 `/v1/chat/completions` 多模态 | 中转站只支持 chat 端点时 |

### 中转站兼容性说明

不同 OpenAI 兼容中转站对 gpt-image-2 的端点支持不同：

| 端点 | 说明 | 注意事项 |
| --- | --- | --- |
| `/v1/images/edits` | **图片编辑**——传入参考图 + prompt，保持布局替换文字。模板克隆模式的最佳选择 | 部分中转站不支持或会直接返回原图 |
| `/v1/images/generations` | **图片生成**——纯 prompt 生成，不支持参考图 | 兼容性最好，几乎所有中转站都支持 |
| `/v1/chat/completions` | **多模态对话**——把参考图作为 image_url 传入，stream 返回生成图片 | 部分中转站走此路径时忽略参考图 |

**推荐做法**：`GPT_IMAGE_ENDPOINT=auto`，脚本会自动尝试最优端点并回退。如果发现某中转站的 edits 端点返回原图不生效，可设 `GPT_IMAGE_ENDPOINT=chat` 强制走 chat 路径。
...
5. 调脚本：
   ```bash
   # 默认模式：图片式PPTX（视觉精美，不可编辑文字）
   python3 scripts/generate_ppt.py --plan slides_plan.json --style styles/editorial-mono.md

   # 可编辑模式：OCR + LAMA修复 + 文本框重建（文字可编辑）
   python3 scripts/generate_ppt.py --plan slides_plan.json --style styles/editorial-mono.md --editable

   # 可编辑快速模式：跳过LAMA修复（更快，但背景保留原始文字）
   python3 scripts/generate_ppt.py --plan slides_plan.json --style styles/editorial-mono.md --editable --skip-inpaint
   ```
6. 产物在 `<cwd>/outputs/<timestamp>/`：
   ...
   - `<title>_editable.pptx` -- **（仅 `--editable` 模式）** 可编辑 .pptx，背景修复 + 文本框可编辑

## 可编辑PPTX模式（--editable）

在生成 AI 图片后，自动调用 [px-image2pptx](https://github.com/JadeLiu-tech/px-image2pptx) 将图片转为可编辑 PPTX：

1. **OCR 文字检测**（PaddleOCR PP-OCRv5）→ 检测文字位置和内容
2. **文字掩码**（经典 CV 双阈值 + OCR 裁剪）→ 精准标记文字像素
3. **LAMA 背景修复**（神经网络修复）→ 擦除文字、还原背景
4. **PPTX 组装**（python-pptx）→ 干净背景 + 可编辑文本框

### 安装可编辑模式依赖

```bash
# 在项目目录克隆 px-image2pptx
...
- `--editable` 模式耗时较长（每页 OCR ~2min + LAMA ~1min），适合最终交付
- `--skip-inpaint` 可跳过 LAMA 修复，速度更快但背景保留原始文字
- 首次运行会自动下载 PaddleOCR 模型（~165MB）和 LAMA 模型（~196MB）
...
1. **先问四件事**（不要直接动手）：
   ...
   - 是否需要**可编辑PPTX**？（默认生成图片式PPTX，加 `--editable` 则通过 OCR + 背景修复生成文本框可编辑的PPTX）
...
|   |---- editable_pptx.py        # 可编辑PPTX转换（调用 px-image2pptx，--editable 模式）
